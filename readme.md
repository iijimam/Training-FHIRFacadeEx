...
どちらもFHIRリソースではない情報からFHIRリソースを作成する流れがあり、その部分は共通の方法で作成しています。（日本法人で作成した「サンプル：JSONテンプレートエンジン」を利用しています）
...
以下2つの流れでご紹介します。（他形式の情報からFHIRリソースのJSONを作成する方法は、ファサードの流れと共通です）
...
- 患者基本情報が含まれるCSVからPatientリソースを作成する流れ
    ...
    プロセスのところで送信されたメッセージを一旦JSONに変換し、JSONテンプレートエンジンを利用してPatientリソースを作成しています。

    FHIRリポジトリへのREST要求を行うオペレーションはシステム提供クラスがあるので、実行時[HS.FHIRServer.Interop.Request](https://docs.intersystems.com/irisforhealthlatest/csp/documatic/%25CSP.Documatic.cls?LIBRARY=HSLIB&CLASSNAME=HS.FHIRServer.Interop.Request)を渡すだけでFHIRリポジトリに処理を依頼できます。

    リポジトリへREST要求を行った結果は、[HS.FHIRServer.Interop.Response](https://docs.intersystems.com/irisforhealthlatest/csp/documatic/%25CSP.Documatic.cls?LIBRARY=HSLIB&CLASSNAME=HS.FHIRServer.Interop.Response)で返送されることが決定しているので、送付するFHIRリソースさえできればリクエストは簡単です。

    工夫の仕方として、CSVのカラムヘッダをテンプレートクラスのプロパティ名に大小文字も含めて合わせておくと、Patientリソースの作成は以下のメソッドの通りとなります。
    [サンプルファイル](/samples/Example-InputDataPatient.csv)
